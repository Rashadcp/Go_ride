...
export function PassengerView({ user, isNotificationsOpen, setIsNotificationsOpen }: any) {
  ...
  const { handleCancelRide, rideRequestLock } = useRideSocket(user);
         ...
         interval = setInterval(() => {
             setSearchCountdown((prev: number | null) => {
                 if (prev !== null && prev <= 1) {
                     ...
                     socket.emit("cancel-ride-request", { passengerId: user?.id });
                     return null;
                 }
  ...
  const handleRequestRide = () => {
      ...
      if (rideRequestLock.current) return;
      rideRequestLock.current = true;

      rideState.setIsRequestingRide(true);
      
      const payload = {
          passengerId: user.id || "passenger_123",
          passengerName: user.firstName ? `${user.firstName} ${user.lastName}` : "Passenger",
  ...
  const handleJoinCarpool = (poolId: string) => {
      socket.emit("carpool:join:request", { poolId, passengerId: user?.id, pickup: userLoc, drop: stops[0]?.coords });
              ...
              {(stops.some((s: any) => s.query || s.coords) || isRouteSearched) ? (
                  <button 
                    onClick={() => { 
                      ...
                      rideState.setIsRouteSearched(false); 
                      rideState.setIsSearchOpen(false); 
                      rideState.setSearchStarted(false); 
                      rideState.setLoadingDrivers(false); 
                      rideState.setVisibleNearbyDrivers([]); 
                      rideState.setRouteInfo({ distance: 0, duration: 0 }); 
                    }} 
      ...
      <div className="absolute top-28 right-8 z-30 w-[420px] h-[calc(100vh-140px)] pointer-events-none flex flex-col justify-start">
                     ...
                     availableCarpools.map((pool) => (
                       ...
                       >
                         <div className="flex items-center gap-4">
                            <div className="w-14 h-14 bg-[#FFD700] text-[#0A192F] rounded-xl flex items-center justify-center shadow-md shrink-0">
                               <Car className="w-8 h-8" />
                            </div>
                            <div className="text-left flex flex-col">
                               <p className="font-extrabold text-[#0A192F] text-[16px]">{pool.driverName || "Community Member"}</p>
                               <div className="flex items-center gap-2 mt-1">
                                  <span className="text-[10px] font-black bg-blue-100 text-blue-600 px-2 py-0.5 rounded-md uppercase tracking-tighter">Shared Ride</span>
                                  <span className="text-[11px] font-bold text-slate-500">{pool.availableSeats} seats left</span>
                               </div>
                            </div>
                         </div>
        ...
        {activeRide && (
          <div className="bg-[#0A192F]/95 backdrop-blur-3xl text-white rounded-[32px] shadow-[0_30px_60px_rgba(0,0,0,0.4)] p-8 border border-[#FFD700]/20 pointer-events-auto shrink-0 relative overflow-hidden">
            ...
            <h3 className="text-[11px] font-black text-[#FFD700] uppercase tracking-[0.3em] mb-2">{activeRide.status === "REQUESTED" ? "Finding you a driver" : activeRide.status === "ACCEPTED" ? "Driver is on the way" : activeRide.status === "ARRIVED" ? "Driver is here" : "Ride in progress"}</h3>
            <p className="text-4xl font-black text-white mb-8 tracking-tighter">{activeRide.eta || 'Calculating...'}</p>
            ...
            {activeRide.driverInfo && (
              <div className="bg-white/5 border border-white/10 rounded-2xl p-5 flex items-center gap-5 mb-8 shadow-inner">
                <div className="w-14 h-14 bg-white rounded-xl flex items-center justify-center overflow-hidden shadow-lg border-2 border-white/20">
                  <Car className="text-[#0A192F] w-7 h-7" />
                </div>
                <div className="flex-1">
                  ...
                  <div className="inline-flex px-3 py-1 bg-[#FFD700]/10 rounded-lg border border-[#FFD700]/20">
                    <p className="text-[#FFD700] font-black text-[11px] tracking-widest">{activeRide.driverInfo.vehiclePlate || "TN 38 AA 1234"}</p>
                  </div>
                </div>
